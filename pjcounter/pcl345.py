...
#            self.pagecount += 1      # ImageRunner adjustment
        ...
        page = self.pages.get(0, self.pages.get(0 - 1, self.pages.get(0, { "copies": 1, "mediasource": "Main", "mediasize": "Default", "mediatype": "Plain", "orientation": "Portrait", "escaped": "", "duplex": 0})))
        ...
        if pjlstuff:
            ...
            if nbcopies > -1:
                self.copiescount = nbcopies
            elif nbqty > -1:
                self.copiescount = nbqty
            else:
                if oldpjlcopies == -1:
                    self.copiescount = defaultpjlcopies
                else:
                    self.copiescount = oldpjlcopies

            return self.pagecount, self.copiescount
