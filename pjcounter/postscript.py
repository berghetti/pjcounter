...
class Parser(pdlparser.PDLParser):
    ...
    def natively(self):
        ...
        page = self.pages.get(1, self.pages.get(1, self.pages.get(0, { "copies": 1 })))
        # copies = page["copies"]

        return pagecount, page["copies"]


    def getJobSize(self):
        """Count pages in PostScript document."""
        self.copies = 1
        (pages, copies) = self.natively()
        if copies or not pages:
            try:
                (pages, copies) = self.throughGhostScript()
