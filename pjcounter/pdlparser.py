...
LOG = logging.getLogger("pjcounter.pdlparser")
    ...
    def isMissing(self, commands):
        ...
        for command in commands:
            if not self.findExecutable(command):
                sys.stderr.write("ERROR: %(command)s is missing or not executable. You MUST install it for pjcounter to be able to do what you want.\n" % locals())
                sys.stderr.flush()
            else:
                howmanythere += 1
